...
`src/embodied.rs` 包含五个边界：

1. `Arena`：位置、方向、食物、危险、能量消耗和终止条件；
2. 感觉编码：能量、当前位置食物、相对食物方向、边界、相对危险方向和最近后果；
3. `AdaptiveController`：固定感觉投影、带泄漏状态与适应项的内部单元；
4. 动作策略：内部状态到四个动作的 softmax 概率；
5. 三因素可塑性：动作误差资格迹乘以后果优势，更新动作突触。
...
`lif.rs`、`network.rs`、`metrics.rs` 和 Gate 0～2 测试仍是经过验证的底层资产，但当前具身控制器不会假装它们已经足以模拟真实神经系统。未来只有在行为闭环中能定义明确对照时，才将电导突触、AdEx、多区室或脉冲可塑性逐项接入。
