...
本文记录已经实现的系统。总研究问题见 [RESEARCH_PROGRAM.md](RESEARCH_PROGRAM.md)；Map 0 建立统一测绘，Map 1 完成单机制负结果，Map 2 加入有界可塑性、持续资源与无重置连续流，冻结边界见 [MAP_0.md](MAP_0.md)、[MAP_1.md](MAP_1.md) 与 [MAP_2.md](MAP_2.md)。
...
- Map 2 的三个串行机制阶段、资源/重置/冻结对照和连续流稳定区域。
...
`src/map2.rs` 复用相同控制器并按阶段注入三类变化。2A 的乘性软边界按权重到边界的剩余空间缩放更新；2B 的每单元资源由持续供给、维护、活动和可塑性成本演化，只统一调制活动；2C 改用跨 720 试次保留活动、权重、奖励基线和资源的连续流。

2C 的规则切换由种子确定且设最小间隔。基线不重置状态，专项对照分别逐试次重置、关闭供能和冻结内部可塑性。Web 读取三份正式 JSON，展示阶段通过状态、配对差值和确认稳定区域，不在浏览器中重新计算形成判据。
