...
因此下一阶段的首要问题不是增加神经元，而是让固定内部表示面对环境规则变化，检验**内部可塑性是否能带来持续适应**。

Gate A 已证明主体在没有距离塑形时仍显著胜过关闭学习，同时确认精确食物方向是主要人工辅助。Gate B 证明短期连续状态有功能贡献并给出 4 步可靠边界；Gate C 证明资格迹能解决受控的 8 步延迟能量归因；Gate D 证明与输入表示对齐的固定稀疏循环能延长记忆；Gate E 完成了连续状态与 LIF 的等预算比较。下一项 Gate F 将冻结环境变化任务，再逐项开放感觉或内部循环可塑性。
