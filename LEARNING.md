...
Map 0/1 已把多个上下文、重复切换、损伤恢复与稳定性纳入统一探针，并连续得到“没有稳定可学习区域”的负结果。因此下一阶段不直接增加神经元或任务复杂度，而先执行[最小动态基座审计](FOUNDATION_AUDIT.md)：依次测试内生有界可塑性、持续基础供能和持续输入。

Gate A 已证明主体在没有距离塑形时仍显著胜过关闭学习，同时确认精确食物方向是主要人工辅助。Gate B 证明短期连续状态有功能贡献并给出 4 步可靠边界；Gate C 证明资格迹能解决受控的 8 步延迟能量归因；Gate D 证明与输入表示对齐的固定稀疏循环能延长记忆；Gate E 完成了连续状态与 LIF 的等预算比较；Gate F 证明冻结动作读出后内部连接可以承担受控规则反转。Map 0/1 则证明这些局部能力尚未组成稳定基座，所以 Gate G 保留与迁移必须等待基座审计通过。
