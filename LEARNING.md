...
因此下一阶段的首要问题不是增加神经元，而是在状态、时间信用分配和循环动力学都有因果基线后，比较**连续状态与脉冲状态在等预算下分别贡献什么**。

Gate A 已证明主体在没有距离塑形时仍显著胜过关闭学习，同时确认精确食物方向是主要人工辅助。Gate B 证明短期连续状态有功能贡献并给出 4 步可靠边界；Gate C 证明资格迹能解决受控的 8 步延迟能量归因；Gate D 又证明与输入表示对齐的固定稀疏循环能把可靠记忆边界延长到 8 步。下一项 Gate E 将冻结统一接口和预算，比较连续状态与 LIF 脉冲状态。
