...
Map 0/1 已把多个上下文、重复切换、损伤恢复与稳定性纳入统一探针，并连续得到“没有稳定可学习区域”的负结果。Map 2 随后完成[最小动态基座审计](FOUNDATION_AUDIT.md)：内生软边界降低漂移，持续供能产生可测行为约束，无重置连续流首次确认 6 点适应区域。该区域只针对当前连续规则任务。Map 2 现作为参考载体，研究重点转向调整规则怎样支持保留、重现和迁移，而不是继续补充节点功能。

Gate A 已证明主体在没有距离塑形时仍显著胜过关闭学习，同时确认精确食物方向是主要人工辅助。Gate B 证明短期连续状态有功能贡献并给出 4 步可靠边界；Gate C 证明资格迹能解决受控的 8 步延迟能量归因；Gate D 证明与输入表示对齐的固定稀疏循环能延长记忆；Gate E 完成了连续状态与 LIF 的等预算比较；Gate F 证明冻结动作读出后内部连接可以承担受控规则反转。下一阶段先测试当前调整机制的多规则保留边界；只有失败类型明确，才单独引入元可塑性、局部调制或结构调整。
