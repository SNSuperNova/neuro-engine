...
第一版采用 LIF（Leaky Integrate-and-Fire，漏积分放电）模型，目的是验证网络级动力学和模拟基础设施，不是复制真实神经元的全部生理过程。Gate E 已将同一 `LifNeuron` 接入延迟线索行为闭环，具体输入编码、低通脉冲读出和等预算对照见 [GATE_E.md](GATE_E.md)。
...
同一神经元、同一时刻的全部输入必须形成一个批次。批内按 `eventId` 排序、求和，然后只进行一次阈值判断；第二个同刻批次属于调度错误并被拒绝。
...
第一版规则：绝对不应期内膜电位保持在复位电位；抵达的突触事件仍记录，但不改变膜电位，也不累积到不应期结束后。到达 `refractoryUntil` 的事件可以正常应用。该规则已有 Gate 0 边界测试；若修改，应创建新的模型版本。
...
Gate 0 已选用微秒作为时间基础单位，并使用 `float64` 膜电位。其余问题解决前，确定性承诺限定为“同一平台、同一构建和同一配置”。跨平台必须能够无损播放同一份已保存事件日志。
