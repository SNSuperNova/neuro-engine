...
use neuro_engine::{M1XConfig, run_m1x_experiment};
...
fn main() {
    ...
    let output = argument(&args, "--output")
        ...
        .unwrap_or_else(|| {
            if quick {
                PathBuf::from("app/public/reachability-development.json")
            } else {
                PathBuf::from("app/public/reachability-v0.9.json")
            }
        });
    let result = run_m1x_experiment(config).expect("M1-X reachability experiment");
    ...
    fs::write(
        &output,
        serde_json::to_string_pretty(&result.published()).expect("serialize published M1-X"),
    )
    .expect("write M1-X artifact");
    ...
    if let Some(raw_output) = argument(&args, "--raw-output").map(PathBuf::from) {
        ...
        fs::write(
            &raw_output,
            serde_json::to_string_pretty(&result).expect("serialize raw M1-X"),
        )
        .expect("write raw M1-X artifact");
