...
use neuro_engine::{StructuralGroupConfig, run_structural_group_diagnostic};
...
fn main() {
    ...
    let output = argument(&args, "--output")
        ...
        .unwrap_or_else(|| {
            if quick {
                PathBuf::from("app/public/structural-group-development.json")
            } else {
                PathBuf::from("app/public/structural-group-v0.6.json")
            }
        });
    let result = run_structural_group_diagnostic(config).expect("M2C-G diagnostic");
    ...
    fs::write(
        &output,
        serde_json::to_string_pretty(&result.published()).expect("serialize published M2C-G"),
    )
    .expect("write M2C-G artifact");
    ...
    if let Some(raw_output) = argument(&args, "--raw-output").map(PathBuf::from) {
        ...
        fs::write(
            &raw_output,
            serde_json::to_string_pretty(&result).expect("serialize raw M2C-G"),
        )
        .expect("write raw M2C-G artifact");
